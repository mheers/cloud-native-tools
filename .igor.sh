IGOR_DOCKER_IMAGE=mheers/cloud-native-tools     # select docker image
IGOR_DOCKER_COMMAND=zsh                         # run this command inside the docker conainer
...
IGOR_DOCKER_NETWORK=host                        # open an interactive tty (0/1)
IGOR_DOCKER_USER=$(id -u)                       # run commands inside the container with this user
# IGOR_DOCKER_USER=0                            # run commands inside the container with this user
IGOR_DOCKER_GROUP=$(id -g)                      # run commands inside the container with this group
IGOR_DOCKER_GROUPS="0 $(getent group docker | awk -F: '{printf "%d", $3}')" # attach multiple groups to the user in the container
...
# IGOR_MOUNTS=""                                # space seperated list of volumes to mount
IGOR_MOUNTS_RO="/var/run/docker.sock ${HOME}/.git/ ${HOME}/.ssh/"          # space seperated list of volumes to mount read only
